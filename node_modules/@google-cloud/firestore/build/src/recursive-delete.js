...
class RecursiveDelete {
     ...
     * @internal
     */
    completionDeferred = new util_1.Deferred();
    /**
    ...
    onQueryEnd() {
        ...
        void this.writer.flush().then(async () => {
            if (this.lastError === undefined) {
                ...
                let error = new (require('google-gax/fallback').GoogleError)(`${this.errorCount} ` +
                ...
                this.completionDeferred.reject(this.lastError.stack
                    ? (0, util_1.wrapError)(error, this.lastError.stack ?? '')
                    : error);
            }
        });
    }
